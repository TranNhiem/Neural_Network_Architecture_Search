...
def compute_x_ticks(x_min, x_max):
    minor_ticks = np.concatenate(
        [np.linspace(0.0, 0.1, 11), np.linspace(0.2, 1.0, 17)])
    x_minor_ticks = minor_ticks[np.logical_and(
        minor_ticks >= x_min, minor_ticks <= x_max)]
    ...
    x_major_ticks = major_ticks[np.logical_and(
        major_ticks >= x_min, major_ticks <= x_max)]
...
def multiple_pareto_fronts(search_state_files, descriptions, y_key=2, take_n=2000,
    ...
    for points, desc, color in zip(point_lists, descriptions, colors):
        ...
        scatter(err, res, label=desc, alpha=(
            0.04 + 0.96 * is_eff), color=color)
        ax.step(err[is_eff], res[is_eff], where="post", alpha=0.7)

    ax.xaxis.grid(True, which='both', linewidth=0.5, linestyle=":")
    ax.yaxis.grid(True, which='major', linewidth=0.5, linestyle=":")
    ax.set_xlabel("Error rate")
    ax.set_ylabel(["Error rate", "Peak memory usage",
                  "Model size", "MACs"][y_key])
    if title:
        ax.set_title(title)
...
def main():
    ...
    p.add_argument("output_file", type=str, default="./out_figures")
    args = p.parse_args()
...
if __name__ == '__main__':
    ...
    #
    plot_pareto_front("/data/home/Rick109/Desktop/Working_space/NAS/NAS_MCU/uNAS/artifacts/AIO_classification/AOI_search_Aging_ALg_agingevosearch_state.pickle",
                      x_range=(0.00, 0.04), y_range=(100, 3_000_000),
                      title="μNAS on AOI w/o mem. usage constraint",
                      output_file="AOI_no_pmu.pdf")

    plot_pareto_front("/data/home/Rick109/Desktop/Working_space/NAS/NAS_MCU/uNAS/artifacts/AIO_classification/AOI_search_Aging_ALg_agingevosearch_state.pickle",
                      x_range=(0.00, 0.04), y_range=(100, 3_000_000),
                      title="μNAS on AOI w/ all constraints",
                      output_file="AOI_all.pdf")
    # multiple_pareto_fronts(
    #         ["artifacts/cnn_mnist/plain_final_agingevosearch_state.pickle",
    #         "artifacts/cnn_mnist/plain_final_bo_search_state.pickle",
    #         "artifacts/cnn_mnist/new_struct_agingevosearch_state.pickle"],
